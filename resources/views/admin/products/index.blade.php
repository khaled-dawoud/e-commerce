...
@section('content')
 ...
 <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a class="btn btn-info" href="{{ route('admin.products.create') }}">Add New Product</a>
 </div>
 ...
 <table class="table table-borderd table-hover table-striped">
     ...
     <tr>
        <td>{{ $product->id }}</td>
        <td>{{ $product->name }}</td>
        <td><img width="100" src="{{ asset('uploads/images/products/'.$product->image) }}"></td>
        <td>{{ $product->price }}</td>
        ...
        <td>
            <a class="btn btn-sm btn-primary" href="{{ route('admin.products.edit' , $product->id) }}"><i class="fa fa-edit"></i></a>
    ...
    @empty
    <tr>
        <td colspan="9" class="text-danger text-center">NO Data Yet </td>
    </tr>
     @endforelse
 </table>
@stop
